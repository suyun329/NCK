...
<div id="dt-area" class="section">
	...
	<div id="dts">
		...
		<div class="sl">
			...
			<p class="pdname"><%=list.get(i).getNckPdName() %></p><!--제품명: pdname-->
			...
			<p class="term">참여자 &nbsp;</p><p class="term"><%= list.get(i).getNckHit() %></p><!--참여자: part--><p>&nbsp;명</p>
			<p class="term">상태 &nbsp; &nbsp;</p><p>진행중</p><!--상태: state-->
			<br>
			<p>판매자 정보</p>
			<p class="term">ID &nbsp;</p> <p><%=list.get(i).getUserID() %></p>
			<p class="term">E-mail &nbsp;</p><p> <%=list.get(i).getUserEmail() %></p>
		</div>
	  ...
	  <%
       ...
       %>
            ...
            <%
                if(pageNumber != 1) {
            %>
                <a href="sale.jsp?pageNumber=<%=pageNumber - 1 %>" class="btn btn-success btn-arrow-left">이전</a>
            <%
                } if (nckDAO.nextPage(pageNumber + 1)) {
            %>
                <a href="sale.jsp?pageNumber=<%=pageNumber + 1 %>" class="btn btn-success btn-arrow-left">다음</a>
